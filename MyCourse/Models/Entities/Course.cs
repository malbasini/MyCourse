    ...
    public partial class Course
    {
        public Course(string title, string author)
        {
            if(string.IsNullOrWhiteSpace(title))
               throw new ArgumentException("Fornire un titolo per la classe Course");
            if(string.IsNullOrWhiteSpace(author))
               throw new ArgumentException("Fornire un Autore per la classe Course");  
            this.Title = title;
            this.Author = author;
            this.Lessons = new HashSet<Lesson>();
            this.CurrentPrice = new Money(Currency.EUR, 0);
            this.FullPrice = new Money(Currency.EUR, 0);
            this.ImagePath = "/Courses/default.png";
        }
